    ...
    <table class="table table-striped jambo_table bulk_action">
        ...
        @foreach ($data as $row)
            <tr class="even pointer">
                ...
                <td class=" "><a href="{{ URL::to($html->base_url.'/'.$row->id.'/edit') }}">{{ $row->first_name.' '.$row->last_name }}</a></td>
                <td class="center"><?= Form::adminPublishButton($row->published, 'users', $row->id); ?></td>
                <td class=" ">{{ $row->id }}</td>
            </tr>
        @endforeach
