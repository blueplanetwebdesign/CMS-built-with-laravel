...
@section('admin::list.content')
    <table class="table table-striped jambo_table bulk_action">
        ...
        <tr class="headings">
            ...
            <th class="column-title column-order"><?= Form::adminGridSort('Layout', 'layout', $list['direction'], $list['order']); ?></th>
        ...
        @foreach ($data as $row)
            <tr class="even pointer">
                ...
                <td class=" ">{{ $row->layout_id }}</td>
                <td class="center"><?= Form::adminPublishButton($row->published, 'categories', $row->id); ?></td>
                <td class=" ">{{ $row->id }}</td>
            </tr>
        @endforeach
