    ...
    <table class="table table-striped jambo_table bulk_action">
        ...
        @foreach ($data as $row)
            <tr class="even pointer">
                ...
                <td class=" "><a href="{{ URL::to($html->base_url.'/'.$row->id.'/edit') }}"><?php LayoutHelper::treePrefix($row->depth) ?>{{ $row->name }}</a></td>
                <td class="center"><?= Form::adminPublishButton($row->published, 'menu', $row->id); ?></td>
                <td class=" ">{{ $row->id }}</td>
            </tr>
        @endforeach
